extends Timer

@export var attack_ability: PackedScene

var s_attack_range = 100


func _on_timeout() -> void:
	var player = get_tree().get_first_node_in_group("player") as Node2D
	if player == null:
		return
	#Фильтруем врагов по дистнации пикселей
	var enemies = get_tree().get_nodes_in_group("enemy")
	enemies = enemies.filter(func(enemy:Node2D):
		return enemy.global_position.distance_squared_to(player.global_position) < pow(s_attack_range, 2)
	)
	if enemies.size() == 0:
		return
	enemies.sort_custom(func(a:Node2D, b:Node2D):
		var a_distnace = a.global_position.distance_squared_to(player.global_position)
		var b_distnace = b.global_position.distance_squared_to(player.global_position)
		return a_distnace < b_distnace
	)
	
	
	var attack_instance = attack_ability.instantiate() as Node2D
	player.get_parent().get_parent().add_child(attack_instance)
	attack_instance.global_position = enemies[0].global_position
